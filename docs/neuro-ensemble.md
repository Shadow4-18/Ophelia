...
5. **Avatar bridge** — VTube Studio / similar via MCP or custom tool
   - **Partial (now):** workstation stage + psyche → Live2D params + VRoid/VRM blendshapes + WS `/api/avatar`
   - **VRoid:** drop a `.vrm` in `~/.ophelia/avatar` — browser loads three-vrm and drives expressions / lip sync
